...
MediaPlayer bgm, sc, sl;
Context context; 
Activity act;
AssetFileDescriptor afd, afdsc, afdsl;
...
void setup()
{
  ...
  if (w>h) {
    ...
    try {
      ...
      afd = context.getAssets().openFd("Escape.mp3");//which is in the data folder
      afdsc = context.getAssets().openFd("Cartoon-Space-Boing.mp3");//which is in the data folder
      afdsl = context.getAssets().openFd("Bottle_Cork.mp3");//which is in the data folder
      
      println("Successfully loaded audio file");
      bgm.setDataSource(afd.getFileDescriptor(), afd.getStartOffset(), afd.getLength());
      bgm.prepare();
      sc.setDataSource(afdsc.getFileDescriptor(), afdsc.getStartOffset(), afdsc.getLength());
      sc.prepare();
      sl.setDataSource(afdsl.getFileDescriptor(), afdsl.getStartOffset(), afdsl.getLength());
      sl.prepare();
    } 
    catch(IOException e) {
